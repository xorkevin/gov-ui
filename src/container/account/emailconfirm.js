...
class AccountEmailConfirm extends Component {
  ...
  async confirmemail(){
    const {err} = await this.props.confirmemail(this.state.key, this.state.password);
    if(err){
  ...
  render({}, {success, err, key, password}){
    ...
    bar.push(<Button primary onClick={this.confirmemail}>Update</Button>);
    return <Card size="md" restrictWidth center bar={bar}>
      <Section subsection sectionTitle="Account Details">
        <Input fullWidth label="code" value={key} onChange={linkstate(this, 'key')}/>
...
AccountEmailConfirm = connect(mapStateToProps, mapDispatchToProps)(AccountEmailConfirm);
AccountEmailConfirm = withRouter(AccountEmailConfirm);
