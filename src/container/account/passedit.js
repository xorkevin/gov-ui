...
class AccountPassEdit extends Component {
  ...
  async editpass(){
    ...
    if(new_password !== password_confirm){
      ...
      const {err} = await this.props.editpass(this.state.old_password, this.state.new_password);
      if(err){
        this.setState((prevState)=>{
  ...
  render({userid}, {success, err, clienterr, old_password, new_password, password_confirm}){
    if(!userid){
      return false;
    }
    const bar = [];
    ...
    bar.push(<Button primary onClick={this.editpass}>Update</Button>);
    return <Card size="md" restrictWidth center bar={bar}>
      <Section subsection sectionTitle="Account Details">
        ...
        <Input fullWidth label="old password" type="password" value={old_password} onChange={linkstate(this, 'old_password')}/>
        <Input fullWidth label="new password" type="password" value={new_password} onChange={linkstate(this, 'new_password')}/>
        <Input fullWidth label="confirm password" type="password" value={password_confirm} onChange={linkstate(this, 'password_confirm')}/>
      </Section>
...
AccountPassEdit = withRouter(AccountPassEdit);
